...
%pixel_reduc is the amount of pixels to be ignored from the perimeter of the ROI in order to remove any partial voluming edge effects 

function [returnSingleValueArray] = CircularAVG(slice, radius, locationX, locationY)
...
sliceAverage = mean2(averages);
            ...
            %Remove data that is greater than a few STDs from the mean
            %Change the value of the variable threshVal to change how many
            %STDs away from the mean to remove

            threshVal = 2;

            singleValueArray = FilterArray(singleValueArray, threshVal);
